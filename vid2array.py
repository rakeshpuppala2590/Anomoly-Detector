...
train_path = "/Users/Abhinavpulikonda/Downloads/Deepvision/training_videos"
fps = 5
train_videos = [f"{str(i).zfill(2)}.avi" for i in range(1, 17)]
...
for video in train_videos:
    ...
    while success:
        frame_path = os.path.join(train_images_path, f"{video[:-4]}_{count:03d}.jpg")
        cv2.imwrite(frame_path, frame)
        success, frame = cap.read()
        count += fps  # Adjust this if you want to capture frames at a different rate
    cap.release()

    images = [file for file in os.listdir(train_images_path) if file.startswith(f"{video[:-4]}_")]
