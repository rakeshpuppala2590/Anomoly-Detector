...
video_path = "/Users/Abhinavpulikonda/Downloads/Deepvision/testing_videos/05.avi"

cap = cv2.VideoCapture(video_path)
print(cap.isOpened())

# Create a folder to save abnormal frames if it doesn't exist
output_folder = "abnormal_frames"
if not os.path.exists(output_folder):
    os.makedirs(output_folder)

frame_count = 0  # Initialize frame count for naming saved frames
...
while cap.isOpened():
    ...
    if 0.00032 < loss < 0.00038:  # Adjusted condition to use range
        print('Abnormal Event Detected')
        # Save the frame where abnormality is detected
        frame_name = f"abnormal_frame_{frame_count}.jpg"
    ...
    cv2.imshow("DeepVision: Real-time Abnormal Activity Detection", resized_frame)
